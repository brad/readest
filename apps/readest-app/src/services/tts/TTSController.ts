...
export class TTSController extends EventTarget {
  ...
  ttsGeminiClient: GeminiTTSClient | null = null;
  ...
  async init() {
    ...
    if (this.ttsGeminiClient && (await this.ttsGeminiClient.init())) {
  ...
  async setVoice(voiceId: string, lang: string) {
    ...
    if (useGeminiTTS) {
      if (!this.ttsGeminiClient) {
        throw new Error('Gemini TTS client is not available');
      }
